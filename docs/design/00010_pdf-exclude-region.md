...
3. 本页受影响且不是 OCR 页 → `lines_to_markdown(&page_items)` 重建(现有行分组逻辑
   `grid_rebuild.rs:122-160` 直接复用);
4. **表格页受影响时的降级**:pdf-inspector 的表格 markdown 是整体产出的,无法按区域剔除,
   此时同样退化为 `lines_to_markdown`,即**丢掉 GFM 表格语法、保留纯文本行**。
   这是明确的取舍,见 §9 待定项 1;P1 通过 `ConvertResult.degradedPages` + 状态栏提示告知用户。
...
1. **表格页降级**:被排除矩形触及的表格页会丢失 GFM 表格语法(退化为纯文本行)。
   是否接受?替代方案是"表格页禁止排除"(更保守)或"整页丢弃"(更激进)。**建议接受降级 + P1 提示**。`@decision`
